...
for name, path in MODELS.items():
    ...
    try:
        ...
        print(f"   └─ 속도: {avg_latency:.1f}ms | 감지율: {detection_rate:.1f}% | 평균 신뢰도: {avg_conf:.1f}%")
        
    except Exception as e:
        print(f"\n❌ {name} 테스트 중 오류 발생: {e}")

print("\n" + "=" * 80)
print("📊 9-Way 데이터셋 전체(77장) 벤치마크 최종 결과")
print("=" * 80)
print(f"{'모델 형식':<24} | {'용량(MB)':<8} | {'속도(ms)':<10} | {'화재 감지율':<12} | {'평균 신뢰도'}")
print("-" * 80)
for r in results:
    print(f"{r['Model']:<24} | {r['Size']:<8} | {r['Latency']:<10} | {r['Rate']:<12} | {r['Conf']}")
print("=" * 80)
