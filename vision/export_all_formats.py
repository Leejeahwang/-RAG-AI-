...
YOLO11N_PT = os.path.join(MODELS_DIR, "best_nano_111.pt")
...
def main():
    ...
    # 4. YOLOv11n (best_nano_111) FP16 변환
    yolo11n_fp16_path = os.path.join(MODELS_DIR, "best_nano_111_openvino_model")
    if os.path.exists(yolo11n_fp16_path):
        print(f"⏭️ YOLOv11n FP16 모델이 이미 존재하여 건너뜁니다: {yolo11n_fp16_path}")
    else:
        export_model(YOLO11N_PT, "openvino", half=True)

    # 5. YOLOv11n (best_nano_111) INT8 변환
    yolo11n_int8_path = os.path.join(MODELS_DIR, "best_nano_111_int8_openvino_model")
    if os.path.exists(yolo11n_int8_path):
        print(f"⏭️ YOLOv11n INT8 모델이 이미 존재하여 건너뜁니다: {yolo11n_int8_path}")
    else:
        if os.path.exists(DATA_YAML):
            export_model(YOLO11N_PT, "openvino", int8=True, data_path=DATA_YAML)
        else:
            print(f"⚠️ 캘리브레이션 데이터셋이 없어 INT8 변환을 기본값으로 진행합니다: {DATA_YAML}")
            export_model(YOLO11N_PT, "openvino", int8=True)
